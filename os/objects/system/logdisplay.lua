...
if args[1] == "update" then
    local response = http.get("https://raw.githubusercontent.com/flakesystems/htos/refs/heads/main/os/objects/system/logdisplay.lua")
    if response then
        local code = response.readAll()
        local file = fs.open("/os/objects/system/logdisplay.lua", "w")
        file.write(code)
        file.close()
    else
